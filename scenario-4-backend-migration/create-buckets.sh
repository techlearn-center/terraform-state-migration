...
if [ "$MODE" = "aws" ]; then
    ...
    echo ""
    echo "IMPORTANT: Update the backend files with your bucket names:"
    echo ""
    echo "1. In backend-a.tf, set: bucket = \"$BUCKET_A\""
    echo "2. In backend-b.tf.example, set: bucket = \"$BUCKET_B\""
    echo ""
    echo "NOTE: For Real AWS, use clean S3 backend config (no LocalStack settings)."
    echo "      See the template comments for the correct format."
    echo ""
